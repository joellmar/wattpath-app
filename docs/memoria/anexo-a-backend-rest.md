...
- `claimOrRegisterDevice` permite reclamar dispositivos sin propietario
  (`user_id = NULL`). El servicio tambien conserva una comprobacion para equipos
  cuyo usuario sea `SYSTEM`, pensada como compatibilidad con datos antiguos o
  seeds previos; no permite reclamar dispositivos de otro usuario real.
