...
### 4.5. Cambios recientes analizados

La documentacion se ha redactado revisando los ultimos commits de `main`. Los
cambios mas relevantes para la memoria son estos:

| Commit | Area | Impacto tecnico documentado |
| --- | --- | --- |
| `ee032fd` | Despliegue | Reinicio explicito de Nginx tras `docker compose up` para evitar respuestas `502` por upstream desactualizado. |
| `d77851b` | Dispositivos y simulacion | Borrado en cascada de lecturas/alertas, telemetria por perfil y panel multi-dispositivo en Angular. |
| `eff9456` | Demo de produccion | Activacion de simuladores y pack de demostracion para probar el dashboard sin hardware fisico. |
| `2db18a4` | Simulacion IoT | Perfiles de consumo, CRUD de dispositivos simulados y generacion de MAC `SIM...`. |
| `3021eba` | Despliegue | Guia Hetzner actualizada con incidencias reales encontradas durante la puesta en marcha. |

Estos cambios explican por que los anexos dan especial peso a `DeviceService`,
`IotTelemetrySimulationJob`, `TelemetryStore`, `DashboardComponent` y a la guia de
despliegue. No se han documentado funcionalidades teoricas: cada apartado apunta a
clases, endpoints o scripts que existen en el repositorio.
