...
catálogo.
También es intencional que `devices.user_id` pueda ser nulo: un dispositivo
físico puede existir antes de ser reclamado por un usuario. Del mismo modo, un
dispositivo recién creado puede no tener lecturas hasta que llegue MQTT o actúe
el simulador.
