...
La store tiene tres tipos de operaciones:

- Métodos síncronos como `setSelectedMac` y `reset`.
- Un helper imperativo, `loadRecentReadings`, que lanza una petición HTTP con
  `subscribe` para rellenar el histórico inicial de la MAC seleccionada.
