...
export default class TariffComponent {
  ...
  readonly activeTariff = signal<any | null>(null);
  ...
  private initPeriods(): void {
    ...
    defaultPeriods.forEach(p => {
      const periodGroup = this.formBuilder.group<PeriodFormGroup>({
        ...
        priceKwh: this.formBuilder.control(0.1500, [Validators.required, Validators.min(0.0001)]),
        ...
        startMonth: this.formBuilder.control(1, [Validators.required, Validators.min(1), Validators.max(12)]),
        endMonth: this.formBuilder.control(12, [Validators.required, Validators.min(1), Validators.max(12)])
      });
  ...
  checkExistingTariff(): void {
    this.http.get<any[]>("/api/v1/tariffs").subscribe({
      ...
      error: () => {
        this.errorMessage.set("No se ha podido comprobar el estado de las tarifas en el servidor.");
      }
    });
  }
  ...
  onSubmit(): void {
    ...
    const payload: TariffRequest = this.tariffForm.getRawValue() as TariffRequest;

    this.http.post("/api/v1/tariffs", payload).subscribe({
      next: (savedTariff) => {
        this.isLoading.set(false);
        this.successMessage.set("Los costes de los tramos horarios se han registrado con éxito.");
        this.activeTariff.set(savedTariff);
        this.tariffForm.reset({ type: '3.0TD', market: 'libre', contractedPowerKw: 15.0, name: '', energyCompany: '' });
      },
      error: (err) => {
        this.isLoading.set(false);
        this.errorMessage.set(err.error?.message || "Error interno en la plataforma al guardar la tarifa.");
      }
    });
  }
  ...
  onDeleteActiveTariff(): void {
    ...
    if (confirm("¿Seguro que deseas eliminar la tarifa activa? Las métricas de costes energéticos se bloquearán de inmediato.")) {
      this.errorMessage.set(null);
      this.successMessage.set(null);

      this.http.delete(`/api/v1/tariffs/${tariff.id}`).subscribe({
        next: () => {
          this.successMessage.set("Tarifa eliminada correctamente. Ya puedes configurar una nueva.");
          this.activeTariff.set(null);
        },
        error: () => {
