...
export default class DevicesComponent {
  ...
  onSubmit(): void {
    ...
    this.http.post<Device>("/api/v1/devices/claim", formValues).subscribe({
      next: () => {
        ...
        this.successMessage.set("El dispositivo ha sido registrado y vinculado correctamente a tu cuenta.");
  ...
  onDelete(id: number): void {
    if (confirm("¿Deseas dar de baja este dispositivo IoT de la red de la empresa?")) {
      this.errorMessage.set(null);
      this.successMessage.set(null);
      ...
      this.http.delete(`/api/v1/devices/${id}`).subscribe({
        ...
        error: () => {
          this.errorMessage.set("No se pudo eliminar el dispositivo de la red.");
        }
      });
    }
  }
  ...
  toggleDeviceStatus(device: Device): void {
    ...
    this.http.put<Device>(`/api/v1/devices/${device.id}`, payload).subscribe({
      ...
      error: () => {
        this.errorMessage.set("Fallo de comunicación al intentar apagar/encender el dispositivo.");
      }
    });
  }
