...
export const TelemetryStore = signalStore(
  ...
  withMethods((
          ...
          return http.post<Device>('/api/v1/devices/claim', payload).pipe(
            tap({
              next: (claimedDevice) => {
                ...
                patchState(store, {
                  devices: updatedList,
                  selectedMac: store.selectedMac() ?? claimedDevice.macAddress
                });
              },
              error: (err) => {
                console.error('error de vinculación en el endpoint claim del backend:', err);
              }
            })
          );
        })
          ...
          return http.post<Device>("/api/v1/devices", newDevice).pipe(
            tap({
              next: (createdDevice) => {
                ...
                patchState(store, {
                  devices: updatedList,
                  selectedMac: store.selectedMac() ?? createdDevice.macAddress
                });
              },
              error: (err) => console.error("error al registrar el dispositivo en el backend:", err)
            })
          );
        })
      )
    ...
    updateDevice: rxMethod<{ id: number; name: string; macAddress: string; isOn: boolean }>(
      pipe(
        switchMap((updatedDevice) => {
          return http.put<Device>(`/api/v1/devices/${updatedDevice.id}`, updatedDevice).pipe(
            tap({
              next: (response) => {
                const updatedList = store.devices().map(d => d.id === response.id ? response : d);
                patchState(store, { devices: updatedList });
              },
              error: (err) => console.error('error al actualizar el medidor en el servidor:', err)
            })
          );
        })
      )
      ...
      pipe(
        switchMap((deviceId) => {
          return http.delete<void>(`/api/v1/devices/${deviceId}`).pipe(
            tap({
              next: () => {
                const updatedList = store.devices().filter(d => d.id !== deviceId);
                const firstMac = updatedList.length > 0 ? updatedList[0].macAddress : null;
                patchState(store, {
                  devices: updatedList,
                  selectedMac: updatedList.some(d => d.macAddress === store.selectedMac()) ? store.selectedMac() : firstMac
                });
              },
              error: (err) => console.error('error al eliminar el dispositivo del sistema:', err)
            })
          );
        })
      )
